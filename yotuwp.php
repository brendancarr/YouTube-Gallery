...
class YotuWP {
	...
	public function init() {
		...
		if ( is_array( $player ) ) {
			foreach ( $this->player as $key => $value ) {
				...
				} elseif ( ! in_array( $key, array( 'width', 'scrolling' ) ) ) {
					$this->player[ $key ] = 'off';
				}
	 ...
	 * Add CSS to header.
	 */
	public function admin_header_css() {
	 ...
	 * Get actions.
	 *
	 * TODO: This should be rewritten in context.
	 */
	...
	public function shortcode( $atts ) {
		...
		if (
			...
			foreach ( $data->items as $video ) {
				...
				$yotuwp_inline_script .= "yotuwp.data.videos['" . $videoId . "'] = " . wp_json_encode( $info, true ) . ';';
	...
	public function load_more() {
		...
		wp_send_json(
			array(
				...
				'items'    => $filtered,
				'settings' => $settings,
				'error'    => ( is_array( $data ) && isset( $data['error'] ) ? $data['error'] : false ),
			)
		);
	...
	public static function getVideoId( $video ) {
		...
		} elseif ( isset( $video->id ) && isset( $video->id->videoId ) ) {
	...
	public function template( $template, $data, $settings ) {
		...
		$overridden_template = locate_template( 'yotu' . YTDS . $template . '.php' );
	...
	public function media_button() {
		...
		if ( stripos( $request_url, 'post.php' ) === false && stripos( $request_url, 'post-new.php' ) === false ) {
			return;
		}
		...
		echo '<a title="' . esc_attr( $button_tip ) . '" title="' . esc_attr( $button_tip ) . '" href="#TB_inline?width=750&height=550&inlineId=yotu_insert_popup" class="yotu-insert-btn button thickbox" >' . wp_kses_post( $icon ) . ' YotuWP</a>';
	...
	public function plugin_redirect() {
		...
		if ( get_option( 'yotuwp_activation_redirect', false ) ) {
			...
			wp_safe_redirect( 'admin.php?page=yotuwp&install=true' );
			die();
		}
	}
	...
	public function get_thumb( $video ) {
		return ( isset( $video->snippet->thumbnails ) && isset( $video->snippet->thumbnails->standard ) )
